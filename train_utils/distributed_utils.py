...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        acc_global = torch.diag(h).sum() / h.sum()  # （TP+TN)/(TP+TN+FP+FN)
        # 计算每个类别的准确率
        acc = torch.diag(h) / h.sum(1)             # TP/(TP+FP)   TN/(TN+FN)
        # 计算每个类别的召回率 
        rec = torch.diag(h) / h.sum(0)          # TP/(TP+FN)   TN/(TN+FP)
        # 计算每个类别预测与真实目标的iou
        iu = torch.diag(h) / (h.sum(1) + h.sum(0) - torch.diag(h))   # TP/(TP+FP+FN)   TN/(TN+FP+FN)?
        return acc_global, acc, rec, iu
    ...
    def __str__(self):
        acc_global, acc, rec, iu = self.compute()
        return (
            'global correct: {:.2f}\n'
            'average precision: {}\n'
            'average recall: {}\n'
